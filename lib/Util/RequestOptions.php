...
class RequestOptions
{
    ...
    public static function parse($options)
    {
        ...
        if (is_array($options)) {
            ...
            if (array_key_exists('Noos_account', $options)) {
                $headers['Noos-Account'] = $options['Noos_account'];
            }
            if (array_key_exists('Noos_version', $options)) {
                $headers['Noos-Version'] = $options['Noos_version'];
            }
            return new RequestOptions($key, $headers);
        }

        $message = 'The second argument to Noos API method calls is an '
           . 'optional per-request apiKey, which must be a string, or '
           . 'per-request options, which must be an array. (HINT: you can set '
           . 'a global apiKey by "Noos::setApiKey(<apiKey>)")';
