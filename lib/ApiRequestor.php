...
class ApiRequestor
{
    ...
    {
        $uaString = 'Noos/v1 PhpBindings/' . Noos::VERSION;

        $langVersion = phpversion();
        $uname = php_uname();

        $appInfo = Noos::getAppInfo();
        $ua = [
            'bindings_version' => Noos::VERSION,
            'lang' => 'php',
        ...
        $defaultHeaders = [
            'X-Noos-Client-User-Agent' => json_encode($ua),
            'User-Agent' => $uaString,
    ...
    private function _requestRaw($method, $url, $params, $headers)
    {
        ...
        if (!$myApiKey) {
            $myApiKey = Noos::$apiKey;
        }

        if (!$myApiKey) {
            $msg = 'No API key provided.  (HINT: set your API key using '
              . '"Noos::setApiKey(<API-KEY>)".  You can generate API keys from '
              . 'the Noos web interface.  See https://noos.app/api for '
              . 'details, or email support@noos.app if you have any questions.';
            throw new Error\Authentication($msg);
        }

        // Clients can supply arbitrary additional keys to be included in the
        // X-Noos-Client-User-Agent header via the optional getUserAgentInfo()
        // method
        $clientUAInfo = null;
        ...
        if (Noos::$apiVersion) {
            $defaultHeaders['Noos-Version'] = Noos::$apiVersion;
        }
