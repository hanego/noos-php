...
class CurlClient implements ClientInterface
{
    ...
    public function request($method, $absUrl, $headers, $params, $hasFile)
    {
        ...
        if (($method == 'post') && (Noos::$maxNetworkRetries > 0)) {
    ...
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to Noos ($url).  Please check your "
                 . "internet connection and try again.  If this problem persists, "
                 . "you should check Noos's service status at "
                 . "https://twitter.com/Noosstatus, or";
                break;
            case CURLE_SSL_CACERT:
            case CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify Noos's SSL certificate.  Please make sure "
                 . "that your network is not intercepting certificates.  "
                 . "(Try going to $url in your browser.)  "
                 . "If this problem persists,";
                break;
            default:
                $msg = "Unexpected error communicating with Noos.  "
                 . "If this problem persists,";
        }
        $msg .= " let us know at support@noos.app.";
    ...
    {
        if ($numRetries >= Noos::getMaxNetworkRetries()) {
            return false;
        }
    ...
    private function sleepTime($numRetries)
    {
        ...
        $sleepSeconds = min(
            Noos::getInitialNetworkRetryDelay() * 1.0 * pow(2, $numRetries - 1),
            Noos::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = max(Noos::getInitialNetworkRetryDelay(), $sleepSeconds);
